...
const { setConversationId, setChatDetails, setErrorMessage, setLoading } = chatDetailsActions;

const useHandleUtterance = async (data, handleUtteranceCallBack) => {
  useAppDispatch(setLoading(true))
  try {
    const response = await client.post("/handle-utterance", data);
    ...
    await handleUtteranceCallBack(successData)
  } catch (error) {
