...
const { setChatDetails,setErrorMessage,setLoading } = chatDetailsActions;
  ...
  const HandleSmartBooking =async (data,handleStartSmartBookingSuccessCallback) =>{
    useAppDispatch(setLoading(true))
    try {
      const response =await client.post("/smart-booking", data);
      ...
      handleStartSmartBookingSuccessCallback(successData)
    } catch (error) {
      console.log("error")
    } finally {
      console.log("finally")
