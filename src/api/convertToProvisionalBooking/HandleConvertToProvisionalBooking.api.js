...
const HandleConvertToProvisionalBooking = async (method, conversationId, context) => {
  useAppDispatch(setLoading(true))
  console.log("hello")
  try {
    let url = `/provisional-booking?conversation_id=${conversationId}`

    if (context) {
      url = `/provisional-booking?conversation_id=${conversationId}&context=${context}`
    }
...
export { HandleConvertToProvisionalBooking };
